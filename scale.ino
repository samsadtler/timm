...
void processBuffer() {
	...
	int changeTracker(){
			if (abs(weight - perviousWeight) <= 2){
					perviousWeight = weight;
					change = false;
					return change;
			} else {
				change = true;
				delta = weight - perviousWeight;
				perviousWeight = weight;
				return change;
			}
	}
	...
	void webHook(){
			...
			Log.info("Weight Sent Via WebHook: %i", weight);
			String lbs = "lbs";
			String form = String::format(
			"{\"weight\":\"%i\",\"units\":\"" +lbs+ "\",\"delta\":\"%d\",\"units\":\"" +lbs+ "\",\"tare\":\"%d\"}",
			weight, delta, tare);
